...
var gumball_endpoint = "http://54.201.23.226:8000/goapi/gumball";
...
var page = function(req, res, state, ts, status) {
  ...
  client.get(gumball_endpoint, args, function(data, response_raw) {
    console.log(data);
    //for(var key in data) {
...
var order = function(req, res, state, ts) {
  ...
  client.post(order_endpoint, args, function(data, response_raw) {
    jsdata = JSON.parse(data);
    for (var key in jsdata) {
      console.log("key:" + key + ", value:" + jsdata[key]);
    }
